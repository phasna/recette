...
async function diagnosticComplet() {
  ...
  try {
    ...
    if (tables.length > 0) {
      ...
      const [structure] = await connection.query("DESCRIBE favorites");
    ...
    console.log(
      `📊 Nombre de favoris dans la base : ${favoritesCount[0].count}`
    );
    ...
    console.log(
      `📝 Nombre de recettes dans la base : ${recipesCount[0].count}`
    );
    ...
    console.log(
      `👥 Nombre d'utilisateurs dans la base : ${usersCount[0].count}`
    );
    ...
    if (usersCount[0].count > 0 && recipesCount[0].count > 0) {
      ...
      console.log(
        "⚠️  Pas assez de données pour tester (besoin d'au moins 1 utilisateur et 1 recette)"
      );
    }
    ...
    try {
      ...
      console.log(
        "   Assurez-vous qu'il est démarré : cd backend && npm start"
      );
    }
